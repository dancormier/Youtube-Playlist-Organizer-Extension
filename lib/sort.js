...
export function normalizeSortOptions(raw) {
  ...
  for (const [key, fallback] of Object.entries(SORT_DEFAULTS)) {
    const allowed = SORT_CHOICES[key].some(choice => choice.value === source[key]);
    out[key] = allowed ? source[key] : fallback;
  }
  return out;
}
 ...
 * or ignores progress, changes the order inside a group, and changes the order
 * of the groups. Unavailable is last whatever the options say.
 *
