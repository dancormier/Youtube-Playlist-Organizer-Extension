...
5. `WLModal` shows the grouped preview. Per-video **treat as unwatched** toggles recompute the order locally — no second Claude call, since cluster assignments don't change
...
4. Toggling "treat as unwatched" moves the video out of the in-progress group without re-calling Claude
